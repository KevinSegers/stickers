...
var stickers = (function($, window, undefined) {
    var init,
        menuOpen = false,
    ...
    resizeHandler = function(){
        ...
        if(windowWidth < 768){
            $('#js-menu-toggle').on(clickType, mobileMenu);
            disableProductImageLink();
        }
    };
    ...
    ie = function(){
        ...
        if (version === false) {
          ...
          document.documentElement.className += ('ie ie--edge');
        } else {
    ...
    showMobileSearch = function(){
        $('.storefront-handheld-footer-bar .search a').on(clickType, function(e) {
    ...
    incrementQty = function(){
        $("input[type=number]").each(function(){
            $(this).addClass('pull-left').after('<div class="increment"><a href="#" class="more">+</a> <a href="#" class="less">-</a></div>');

            var links = $(this).next().find("a");

            $(links).on(clickType, function(e) {
                var buttonContainer = $(this).parent(),
                    button = $(this),
                    input = $(buttonContainer).prev();
                    oldValue = $(input).val();
        ...
        // make btn re-apear when cart is reloaden with ajax
        $(function() {
            $.ajaxSetup({
    ...
    homepageSlider = function(){
        var gallery = $('.homepage-slider').flickity({
    ...
    stickerGallery = function(){
        var gallery = $('.sticker-gallery__images-inner').flickity({
            freeScroll: true,
            wrapAround: true,
            prevNextButtons: false,
            imagesLoaded: true,
    ...
    disableProductImageLink = function(){
        var link = $('.images__inner').find("a");
        $(link).removeClass('zoom').removeAttr('href').removeAttr('data-rel').on(clickType, function(e) {
            e.preventDefault();
            return false;
            e.stopPropagation();
        });
    ...
    //     $(trigger).on(clickType, function(e) {
    //         $('.login').toggle();
    //         e.preventDefault();
    //     });
    // }
    ...
    mobileMenu = function(e){
        var navWrapper = $('#js-nav-wrapper'),
            menuBtn = $('#js-menu-toggle'),
            activeClass = 'is-active',
            activeBodyClass = 'menu-is-active',
            body = $('body');

        // $(menuBtn).off(clickType, mobileMenu);
        $(menuBtn).off(clickType, mobileMenu).on(clickType, mobileMenu);
        // Check to see which loop to use.
        if(!menuOpen){
            // console.log('open');
            $(navWrapper).fadeIn(200);
            $(menuBtn).addClass(activeClass);
            $(body).addClass(activeBodyClass);
            setTimeout(function(){
                menuOpen = true;
                $(menuBtn).on(clickType, mobileMenu);
                // console.log('done waiting after open');
            }, 100);
        }
        else{
            // console.log('close');
            $(navWrapper).fadeOut(200);
            $(menuBtn).removeClass(activeClass);
            $(body).removeClass(activeBodyClass);
            setTimeout(function(){
                menuOpen = false;
                $(menuBtn).on(clickType, mobileMenu);
                // console.log('done waiting after close');
            },100);
        }
        e.preventDefault();
    };
